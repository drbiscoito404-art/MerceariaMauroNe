...
function updateCartBadge() {
  ...
  if (total <= 0) {
    ...
    elements.cartBadge.textContent = total.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
  }
}
...
function renderCartDropdown() {
  ...
  content.innerHTML = '';
  if (!state.user) {
    content.innerHTML = '<p>Faça login para ver o carrinho.</p>';
    return;
  }
  ...
  if (!state.user.cart || state.user.cart.length === 0) {
    content.innerHTML = '<p>Seu carrinho está vazio.</p>';
    return;
  }
...
function renderProducts() {
  ...
  if (state.data.merchandise.length === 0) {
    elements.productList.innerHTML = '<p>Nenhuma mercadoria cadastrada.</p>';
    return;
  }
  state.data.merchandise.forEach(item => {
    ...
    card.innerHTML = `
      ...
      <p>${item.promotion || 'Sem promoção'}</p>
...
function renderEmployees() {
  ...
  if (state.data.employees.length === 0) {
    elements.employeeList.innerHTML = '<p>Nenhum funcionário cadastrado.</p>';
    return;
  }
...
function renderAdminLists() {
  ...
  if (!state.data) return;
  if (state.data.merchandise.length === 0) {
    elements.adminProductList.innerHTML = '<p>Nenhuma mercadoria cadastrada.</p>';
  } else {
    state.data.merchandise.forEach(item => {
      const card = document.createElement('div');
      card.className = 'admin-card';
      card.innerHTML = `
        <label>Nome <input data-field="product-name" value="${item.name}" /></label>
        <label>Preço <input data-field="product-price" type="number" value="${item.price}" /></label>
        <label>Quantidade <input data-field="product-quantity" type="number" value="${item.quantity}" /></label>
        <label>Promoção <input data-field="product-promotion" value="${item.promotion || ''}" /></label>
        <div class="cart-item-controls">
          <button class="qty-button" data-admin-action="save-product" data-id="${item.id}">Salvar</button>
          <button class="remove-button" data-admin-action="remove-product" data-id="${item.id}">Remover</button>
        </div>
      `;
  ...
  if (state.data.employees.length === 0) {
    elements.adminEmployeeList.innerHTML = '<p>Nenhum funcionário cadastrado.</p>';
  } else {
    state.data.employees.forEach(emp => {
      const card = document.createElement('div');
      card.className = 'admin-card';
      card.innerHTML = `
        <label>Nome <input data-field="employee-name" value="${emp.name}" /></label>
        <label>Cargo <input data-field="employee-role" value="${emp.role}" /></label>
        <label>Foto URL <input data-field="employee-image" value="${emp.imageUrl || ''}" /></label>
        <div class="cart-item-controls">
          <button class="qty-button" data-admin-action="save-employee" data-id="${emp.id}">Salvar</button>
          <button class="remove-button" data-admin-action="remove-employee" data-id="${emp.id}">Remover</button>
        </div>
      `;
...
function renderLocation() {
  ...
  if (!state.data) return;
  if (locationText) locationText.textContent = state.data.location || '';
  if (locationDescription) locationDescription.textContent = state.data.locationDescription || '';
  if (serviceDescription) serviceDescription.textContent = state.data.serviceDescription || '';
}
...
async function doLogin(event) {
  ...
  if (!name) {
    showMessage('Nome obrigatório.', elements.authMessage);
    return;
  }
  if (!isValidEmail(email)) {
    showMessage('E-mail inválido.', elements.authMessage);
    return;
  }
  const response = await fetch('/api/login', {
    method: 'POST',
  ...
  if (!response.ok) {
    showMessage(result.error || 'Falha no login.', elements.authMessage);
    return;
  }
...
async function doRegister(event) {
  ...
  if (password.length < 6) {
    showMessage('A senha deve ter pelo menos 6 caracteres.', elements.authMessage);
    return;
  }
  if (password !== confirmPassword) {
    showMessage('Senhas não conferem.', elements.authMessage);
    return;
  }
  ...
  const result = await response.json();
  if (!response.ok) {
    showMessage(result.error || 'Falha no cadastro.', elements.authMessage);
    return;
  }
  state.user = { name: result.name || name, email: result.email, password, cart: result.cart || [] };
  saveSession(state.user);
...
async function unlockAdmin(event) {
  ...
  if (!response.ok) {
    showMessage('Código administrativo incorreto.', elements.adminLockMessage);
    return;
  }
  state.adminUnlocked = true;
  elements.adminPanel.classList.remove('hidden');
  elements.adminLockPanel.classList.add('hidden');
  showMessage('Acesso admin liberado.', elements.adminLockMessage, 'success');
}
...
async function addProduct(event) {
  ...
  if (!name) {
    showAdminMessage('Nome é obrigatório.', 'error');
    return;
  }
  ...
  if (!response.ok) {
    const result = await response.json();
    showAdminMessage(result.error || 'Falha ao adicionar mercadoria.', 'error');
    return;
  }
  await updateData();
  event.target.reset();
...
async function addEmployee(event) {
  ...
  if (!name || !role) {
    showAdminMessage('Nome e cargo são obrigatórios.', 'error');
    return;
  }
  ...
  if (!response.ok) {
    const result = await response.json();
    showAdminMessage(result.error || 'Falha ao adicionar funcionário.', 'error');
    return;
  }
  await updateData();
  event.target.reset();
...
async function changeCode(event) {
  ...
  if (!code) {
    showAdminMessage('Código é obrigatório.', 'error');
    return;
  }
  ...
  if (!response.ok) {
    const result = await response.json();
    showAdminMessage(result.error || 'Falha ao atualizar código.', 'error');
    return;
  }
...
async function changeLocation(event) {
  ...
  if (!location) {
    showAdminMessage('Localização é obrigatória.', 'error');
    return;
  }
  ...
  if (!response.ok) {
    const result = await response.json();
    showAdminMessage(result.error || 'Falha ao atualizar localização.', 'error');
    return;
  }
  await updateData();
...
async function handleRootClicks(event) {
  ...
  if (removeButton && removeButton.dataset.id) {
    ...
    if (removeButton.dataset.adminAction) return;
    if (state.user) {
  ...
  if (adminActionButton) {
    ...
    if (action === 'save-product') {
      const card = adminActionButton.closest('.admin-card');
      await saveProductEdit(id, card);
    }
    ...
    if (action === 'save-employee') {
      const card = adminActionButton.closest('.admin-card');
      await saveEmployeeEdit(id, card);
    }
...
async function init() {
  ...
  document.addEventListener('click', event => {
    if (!event.target.closest('#cart-dropdown') && !event.target.closest('#cart-toggle')) {
      closeCartDropdown();
    }
  });
